...
for i = 1:length(rps)
    ...
    fname = strcat(subid, '_', runid, '_rms_drift_2015.txt');
    fname = char(fname);
    
    rparams = load(rps{i});
    rparams(:, 4:6) = 50 .* rparams(:, 4:6);
    ...
    maxvalue = max(rpsumsqrt);
    meanvalue = mean(rpsumsqrt);
    ...
    save(fname, 'rpsumsqrt', '-ascii');
end

T = cell2table(rmsstats,'VariableNames',{'SubID', 'RunID', 'Max', 'Mean'});
writetable(T, 'totalstats_drift.txt');